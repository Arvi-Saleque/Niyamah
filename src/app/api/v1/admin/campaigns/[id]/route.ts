import type { NextRequest } from "next/server";
import { campaignRepository } from "@/modules/marketing/infrastructure/campaign.repository";
import { campaignUpdateSchema } from "@/lib/validations/marketing";
import { apiSuccess, apiError } from "@/lib/utils/api-response";
import { requirePermission } from "@/modules/auth/application/get-admin-access";

interface Ctx {
  params: Promise<{ id: string }>;
}

export async function GET(_req: NextRequest, { params }: Ctx) {
  const guard = await requirePermission("campaigns.view");
  if ("error" in guard) return guard.error;
  const { id: idStr } = await params;
  const id = Number(idStr);
  if (!Number.isInteger(id) || id <= 0) return apiError("INVALID_ID", "Invalid id.", 400);
  const row = await campaignRepository.findById(id);
  if (!row) return apiError("NOT_FOUND", "Campaign not found.", 404);
  return apiSuccess(row);
}

export async function PATCH(req: NextRequest, { params }: Ctx) {
  const guard = await requirePermission("campaigns.manage");
  if ("error" in guard) return guard.error;
  const { id: idStr } = await params;
  const id = Number(idStr);
  if (!Number.isInteger(id) || id <= 0) return apiError("INVALID_ID", "Invalid id.", 400);
  const body = await req.json().catch(() => null);
  if (!body) return apiError("INVALID_JSON", "Invalid body.", 400);
  const parsed = campaignUpdateSchema.safeParse(body);
  if (!parsed.success) {
    return apiError("VALIDATION_ERROR", "Invalid input.", 422, parsed.error.flatten().fieldErrors);
  }
  const updated = await campaignRepository.update(id, parsed.data);
  if (!updated) return apiError("NOT_FOUND", "Campaign not found.", 404);
  return apiSuccess(updated);
}

export async function DELETE(_req: NextRequest, { params }: Ctx) {
  const guard = await requirePermission("campaigns.manage");
  if ("error" in guard) return guard.error;
  const { id: idStr } = await params;
  const id = Number(idStr);
  if (!Number.isInteger(id) || id <= 0) return apiError("INVALID_ID", "Invalid id.", 400);
  const ok = await campaignRepository.remove(id);
  if (!ok) return apiError("NOT_FOUND", "Campaign not found.", 404);
  return apiSuccess({ deleted: true });
}
