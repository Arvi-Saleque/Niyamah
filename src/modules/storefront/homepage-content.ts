...
// ─────────────────────────────────────────────
// Types — kept primitive (icon = string key) so JSON-safe.
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// Defaults — copied from current hardcoded values.
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// Fetcher — DB rows merged over defaults. Inactive blocks fall back to default.
// ─────────────────────────────────────────────
...
export async function getWhatsAppConfig(): Promise<WhatsAppData> {
  const [row] = await db
    ...
    .where(eq(homepageBlocks.storeId, DEFAULT_STORE_ID))
    .limit(50);
  // Cheap path — single query, find whatsapp
  const all = await db
    .select()
    .from(homepageBlocks)
    .where(eq(homepageBlocks.storeId, DEFAULT_STORE_ID));
...
/** Admin: list all blocks with status, merged with defaults so missing ones still show. */
export async function getAllHomepageBlocksAdmin() {
  ...
  return HOMEPAGE_BLOCK_KEYS.map((key) => {
    ...
    return {
      blockKey: key,
      data: row?.data ?? HOMEPAGE_DEFAULTS[key],
